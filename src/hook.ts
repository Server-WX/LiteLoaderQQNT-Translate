...
const HookApiPreProcesses: {
  ...
  hookFn: (payload: unknown[]) => Promise<unknown[]>,
}[] = [];
...
export function hookQQNTApiCall(window: BrowserWindow) {
  ...
  const ipcUpMsgProxy = new Proxy(ipcUpMsg, {
    async apply(target, thisArg, args: IQQNTApiPost) {
...
export function addApiPreProcess(postCommand: EQQNTApiCommand, hookFn: (payload: unknown[]) => Promise<unknown[]>): number {
